...
def fit_ownwcoef(target, modelpath, band):
    ...
    for jj in (np.arange(norders)):
        ...
        hilim = wls[:, jj, :].max() + 0.003
        tind = (model['wl']>lolim) * (model['wl'] < hilim)
        _lam_template = model['wl'][tind]
        _template = model['flux'][tind]
        _template /= np.median(_template)
    
        # to be compatible with rotational profile convolution kernel
        if len(_lam_template) < 400:
           new_lam = np.linspace(_lam_template[0], _lam_template[-1], 400)
           _template = np.interp(new_lam, _lam_template, _template)
           _lam_template = new_lam

        # fit continuum coefficients / flux scaling
        ccoef = [-0.1, 1.2/np.median(_template)]
        NPC = len(ccoef)
        ind90 = np.sort(fobs0[jj])[int(0.9*npix)]  
        ccoef = np.polyfit(pix[fobs0[jj]>ind90], fobs0[jj][fobs0[jj]>ind90], NPC-1)
        ...
        for obs in np.arange(nobs):
            ...
            lam_template = np.polyval(wcoef, pix)
            template = np.interp(lam_template, _lam_template, _template)

            guess = np.concatenate(([21, 0.3, 9e-5], ccoef))
            fitargs = (mf.modelspec_template, lam_template, template, NPC, npix, fobs0[jj], wfobs0[jj])
    ...
    if "Callie" in modelpath:
        resultdir = f"{homedir}/uoedrive/result/Callie"
    results.write(f'{resultdir}/IGRINS2_{target}_{band}_fitting_results_{modelname[:12]}.txt', format='ascii', overwrite=True)
    fits.writeto(f'{resultdir}/IGRINS2_{target}_{band}_chipmods_{modelname[:12]}.fits', chipmods, overwrite=True)
    fits.writeto(f'{resultdir}/IGRINS2_{target}_{band}_chiplams_{modelname[:12]}.fits', chiplams, overwrite=True)
