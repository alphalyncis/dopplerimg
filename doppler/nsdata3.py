...
# Original author: Ian Crossfield (Python 2.7)
 ...
 2018-07-06 13:14 IJMC: Made compatible with Python3 (I think)
...
def getirafap(filename, filelist=False, verbose=False):
    ...
    def apapply(ap, filestr, keyword):
        ...
        if filestr.find(keyword)!=-1:
            ...
            exec(  'ap.' + keyword + '.append(map(float, filestr.split()))' ) # EB updated exec statement
        return ap 
...
"""
    ...
    if date=='2016oct20b':
        planet = 'WASP-33 b'
        prefix = 'spec_'
        framelist = range(118,192)
        flatlist = range(42,58)
        darklist = range(193,198)
        callist = range(118,122)
        datadir = '20161020b/'
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_20161020b_spec_'
        n_aperture = 7
        filter = 'Karies'
        calnod = False
        ...
        planet = 'WASP-33 b'
        prefix = 'spec_'
        framelist = range(111,140) + range(141,206)
        flatlist = range(70,89)
        darklist = range(208,215)
        callist = range(111,114)
        datadir = '20161019/'
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_20161019_spec_'
        n_aperture = 7
        filter = 'Karies'
        calnod = False
        
    elif date=='2016oct15b':
        planet = 'WASP-33 b'
        prefix = 'spec_'
        framelist = range(173, 217)
        flatlist = range(30,65)
        darklist = range(230, 233)
        callist = range(165,170)
        datadir = '20161015a/'
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_20161015a_spec_'
        n_aperture = 7
        filter = 'Karies'
        calnod = False
        ...
        planet = 'HD 187123 b'
        prefix = 'spec_'
        framelist = range(154,160)
        flatlist = range(30,65)
        darklist = range(222,227)
        callist = range(165,170)
        datadir = '20161015a/'
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_20161015a'
        n_aperture = 19
        filter = 'Karies'
        calnod = False
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_hatp11_2014sep17_sep17s'
        n_aperture = 7
        filter = 'K'
        calnod = False
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_wasp12_2010apr22_ns_20100422'
        n_aperture = 6
        filter = 'K'
        calnod = False
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_55cnc_2008mar21_mar21s'
        n_aperture = 6
        filter = 'K'
        calnod = False
        rowfix = ['a']
        ...
        ap_suffix = 'undetermined' #'database/ap_' + _aphome + '_proj_pcsa_data_proc_gj1214_2010aug16_aug16s'
        n_aperture = 6
        filter = 'SXD'
        calnod = False
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_gj1214_2010aug16_aug16s'
        n_aperture = 7
        filter = 'K'
        calnod = False
        ...
        datadir = 'gj1214_2010aug16/'
        ap_suffix = 'ec/ap_' + _aphome + '_proj_pcsa_data_proc_gj1214_2010aug16_aug16s'
        n_aperture = 7
        filter = 'K'
        calnod = False
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_gj1214_2010sep04_sep04s'
        n_aperture = 8
        filter = 'H'
        calnod = False
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_hd209458_2009jul29_jul29s'
        n_aperture = 6
        filter = 'K'
        calnod = False
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_hd189733_2008jun15_jun15s'
        n_aperture = 6
        filter = 'K'
        calnod = True
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_tauboo_2008jun15_jun15s'
        n_aperture = 6
        filter = 'K'
        calnod = False
        ...
        ap_suffix = 'database/ap_' + _aphome + '_proj_pcsa_data_proc_hd189733_2008jul12_jul12s'
        n_aperture = 5
        filter = 'L'
        calnod = True
...
def readart(filename, ignore='***'):
    ...
    def process(struct, valstr, names, types, delim=','):
        ...
        for val, name, type in zip(vals, names, types):
            if type=='DBL':
                exec( 'struct.%s.append(float(val.strip()))' % name in locals() ) # EB updated exec statement
            else:
                exec( 'struct.%s.append(val.strip())' % name in locals() ) # EB updated exec statement
        return
    ...
    for name in entry_names:
        exec( 'output.%s = []' % name in locals() ) # EB updated exec statement
...
def readnstemps(filename):
    ...
    for val, name in zip(vals, valnames):
        exec( 'output.%s = [val]' % name in locals() ) # EB updated exec statement

    for line in f:
        vals = map(float, line[pos0::].strip().split(' '))        
        output.time.append(line[0:pos0])
        for val, name in zip(vals, valnames):
            exec( 'output.%s.append(val)' % name in locals() ) # EB updated exec statement
...
def readarm(filename):
    ...
    def process(struct, line, delim=','):
        ...
        if keyval=='***':
            ...
            try:
                exec( 'struct.%s.append(keyval)' % keyname in locals() ) # EB updated exec statement
                exec( "struct.%sHST.append(HSTdatetime)" % keyname in locals() ) # EB updated exec statement
            except: 
                exec( 'struct.%s = [keyval]' % keyname in locals() ) # EB updated exec statement
                exec( "struct.%sHST = [HSTdatetime]" % keyname in locals() ) # EB updated exec statement
